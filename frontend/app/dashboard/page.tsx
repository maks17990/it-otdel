...
export default function Dashboard() {
  ...
  const stats = [
    ...
    {
      icon: <FolderKanban className="text-yellow-400 w-6 h-6" />,
      label: 'Заявки',
      value:
        requestsCount === null
          ? 'Загрузка...'
    ...
    {
      icon: <Newspaper className="text-green-400 w-6 h-6" />,
      label: 'Новости',
      value: '3 новых',
      onClick: () => router.push('/news')
    },
    {
      icon: <FileWarning className="text-pink-400 w-6 h-6" />,
      label: 'Инструкции',
      value: 'Обновлено вчера',
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-7 mb-12">
          {stats.map((s, i) => (
            <button
              key={i}
              onClick={s.onClick}
              className="bg-white/10 border border-cyan-200/10 backdrop-blur-xl p-7 rounded-2xl shadow-lg flex items-start gap-6 hover:bg-cyan-400/10 hover:shadow-2xl transition-all duration-200 w-full text-left group"
            >
              <span className="bg-cyan-500/10 p-3 rounded-full">{s.icon}</span>
              <div>
                <p className="text-sm text-cyan-100/70">{s.label}</p>
                <p className="text-xl font-semibold mt-1 text-white/90 group-hover:text-cyan-200 transition">{s.value}</p>
              </div>
            </button>
          ))}
        </div>
